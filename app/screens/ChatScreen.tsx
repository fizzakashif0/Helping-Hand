...
export default function ChatScreen({ route }: any) {
  ...
  const fetchThread = async () => {
    try {
      setLoading(true);
      const data = await chatApi.getThreadById(threadId, userId);
      setThread(data);
      ...
      // Fetch messages
      const messagesData = await chatApi.getMessages(threadId, userId);
      setMessages(messagesData);

      // Mark as read
      socketService.markRead(threadId);
    } catch (error) {
  ...
  const handleSendMessage = () => {
    ...
    socketService.sendMessage(threadId, inputText.trim());
    setInputText("");
    socketService.emitStopTyping(threadId);
  };

  const handleTyping = () => {
    socketService.emitTyping(threadId);
