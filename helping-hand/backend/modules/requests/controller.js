...
exports.applyForDonation = async (req, res) => {
  try {
    ...
    // Update donation status + applicants list
    donation.status = "matched";
    if (request.requester) {
      const requesterId = request.requester;
      const requesterIdStr = String(requesterId);
      const hasApplicant = (donation.applicants || []).some(
        (a) => String(a) === requesterIdStr
      );
      if (!hasApplicant) {
        donation.applicants = donation.applicants || [];
        donation.applicants.push(requesterId);
      }
    }
    await donation.save();
